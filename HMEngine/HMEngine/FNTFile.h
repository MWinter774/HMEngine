	...
	namespace Core
	{
		...
		public:
			typedef struct
			{
				...
				glm::vec2 offsets;    // Offset from baseline to left/top of glyph
				GLuint     advance;    // Offset to advance to next glyph
			} Glyph;

			FNTFile(const std::string& ttfFilePath);
			~FNTFile();
			FNTFile(const HMEngine::Core::FNTFile& other) = delete;
			HMEngine::Core::FNTFile& operator=(const HMEngine::Core::FNTFile& other) = delete;

			inline std::string GetTTFFilePath() const { return this->_ttfFilePath; }

			HMEngine::Core::FNTFile::Glyph GetCharacter(char c);

		private:
			std::string _ttfFilePath;
			FT_Library _fontLibrary;
			FT_Face _font;

			std::unordered_map<char, HMEngine::Core::FNTFile::Glyph> _glyphs;
		};
	}
} 